...
if (Input::exists()) {
   ...
   if ($validation->passed()) {
      try {
         $res = $admin->login(Input::get('username'), Input::get('password'));
         if ($res) {
            Session::put('admin_id', $res->first()->id);
         ...
         <div class="authincation-content mx-auto shadow rounded" style="width: 400px;">
            <div class="auth-form py-4 text-center">
               ...
               <form method="POST" class="mt-4">
                  <div class="mb-3">
                     <input type="text" class="form-control border-secondary" placeholder="Admin Username" name="username" id="username" value="<?= Input::get('username') ?>">
                  </div>
                  <div class="mb-3">
